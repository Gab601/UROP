...
public class OpenCvTest {
    ...
    public static void main (String args[]) {
        ...
        String strokeTimePath = "/home/gkammer/Downloads/Albedo1.csv";
        String videoPath = "/home/gkammer/Videos/Albedo1Trim.mp4";
        int maximumStrokes = 0;
        ...
        ArrayList<double[]> strokeTimes = CreateStrokeArrayFromFile(strokeTimePath);
        int currentFrame = 0;
        int currentStrokeNum = 0;

        VideoCapture videoCapture = new VideoCapture(videoPath);
        for (double[] time: strokeTimes) {
            if (currentStrokeNum >= maximumStrokes) {
                break;
            }
            currentStrokeNum++;
            int startFrame = (int)(time[0]*framerate);
            int endFrame = (int)(time[1]*framerate);
            ...
            ArrayList<BufferedImage> videoArrayList = new ArrayList<>();
            ...
            while (currentFrame < endFrame) {
                if (videoCapture.read(mat)) {
                    if (height == 0 || width == 0) {
                        height = mat.rows()/compressionFactor;
                        width = mat.cols()/compressionFactor;
                    }
                    rawImage = Mat2Image(mat);
                    videoArrayList.add(BlurImage(CompressImage((BufferedImage)rawImage, compressionFactor)));
                    currentFrame++;
                }
                else {
                    System.out.println("Unable to read from file!");
            ...
            BufferedImage finalImage = CreateMovementImage(videoArrayList, outlierTolerance, skinColor, 5, background);
            String saveFileLocation = "/home/gkammer/Pictures/UROP_Strokes/" + time[0] + "-" + time[1] + background.toString() + "albedo.png";
            File outputfile = new File(saveFileLocation);
            try {
                ImageIO.write(finalImage, "png", outputfile);
                System.out.println("Saved file at location " + saveFileLocation);
            }
            catch (IOException e) {
                e.printStackTrace();
            }
